...
| ~~Amazon MSK mode~~ | **Closed by ADR-035.** MSK is removed; Kafka is self-hosted on AWS as a container. Revisit only if budget expands or managed-broker behavior becomes the subject of study. | — | — | — |
...
| --- | --- | --- |
| **Istio ambient, KEDA, Argo Rollouts, Argo CD** *(stack reduction, ADR-036)* | Platform and SRE responsibilities. Backend postings essentially never require service mesh, queue-driven autoscaling, progressive delivery, or GitOps reconciliation. Among the largest lifts in the plan for the least job-relevant learning. | A specific role or requirement genuinely demands one. Reintroduce singly, with the learning outcome it serves named first. |
| **Jenkins** *(stack reduction, ADR-039; reverses ADR-022-R1)* | A JCasC controller with ephemeral Kubernetes agents is one of the largest single lifts in the plan, for a tool declining in postings, teaching a build-versus-deploy separation that GitHub Actions teaches in an afternoon. | An enterprise role specifically requires it. Knowing what Jenkins is and how it differs from hosted CI is sufficient until then. |
| **Strimzi and RabbitMQ Cluster Operator** *(stack reduction, ADR-036)* | The goal is to learn Kafka and RabbitMQ. Operator lifecycle management is a distinct Kubernetes-administration skill that teaches nothing about partitions, ordering, consumer groups, or delivery semantics. | Broker lifecycle in Kubernetes becomes the actual subject of study, rather than a prerequisite to using the brokers. |
| **Loki and Tempo** *(stack reduction, ADR-039)* | Two more stores to operate. Prometheus and Grafana make partial failure visible enough for every acceptance requirement in [03](03-delivery-roadmap.md). | The stack has room and metrics prove insufficient for a real diagnosis. Distributed tracing is the first thing to add back. |
| **Spring Cloud Gateway and `api-gateway`** *(stack reduction, ADR-038)* | An extra deployable, image, chart, and deployment for a pattern better learned once the system exists. Edge concerns are not the lesson here. | A cross-cutting edge concern appears that cannot be handled per-service, or the gateway pattern itself becomes the study subject. |
| **Traefik and Gateway API as study items** *(stack reduction, ADR-036)* | Use whatever k3d ships with. The cluster edge is plumbing, not a learning outcome. | Ingress behavior becomes the actual subject of study. |
| **MSK, Amazon MQ, Redis Cloud, AMP, Managed Grafana, CloudFront, OpenSearch Service** *(stack reduction, ADR-035)* | Budget. Managed brokers alone would consume the entire $100 credit allowance. Self-host what is already known operationally from local work. | Credits or budget expand, or a specific managed-service behavior becomes the subject. One short comparison exercise is enough to speak to what managed buys. |
| **EKS as the primary AWS runtime** *(stack reduction, ADR-035)* | ~$73/month control plane before any compute, against $100 in credits. Conflates learning Kubernetes with learning AWS; k3d teaches Kubernetes for free. | Not excluded outright — retained as **one bounded exercise** at B3. Making it the primary target again requires a budget that supports it. |
...
| **Docker Compose, in any role** *(re-excluded, ADR-037)* | The narrow Phase A aid that ADR-017-R1 permitted is withdrawn. k3d runs from stage A1, so a second environment would exist only to be migrated off, and would let Compose-only assumptions leak into application code. Ordinary tests use Testcontainers and need no environment at all. | Cluster friction genuinely blocks correctness work — and even then the recorded fallback is Testcontainers alone with in-cluster deployment deferred to the end of the stage, **not** reintroducing Compose. |
