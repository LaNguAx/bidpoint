...
Auction close/cancel has a stronger internal REST contract. Core `auctions` first commits `CLOSING`, then calls the `bidding-service` fence/finalize command with a stable command identity, reason, authoritative `closeAt`, and monotonic lifecycle version. The command is idempotent: it serializes with bid acceptance, persists the fence plus final outcome, and returns the same acknowledgement on retry. A timeout does not authorize Core to infer completion; Core remains `CLOSING` and retries the same identity, committing `CLOSED`/`CANCELLED` and later settlement only after acknowledgement. Bid requests use trusted server/database time, ignore client time, and reject at or after `closeAt`; opening projection lag may reject, but close lag may never accept.
